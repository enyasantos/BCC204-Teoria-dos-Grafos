...
GraphBFS::GraphBFS(int vertices, int arestas, int tipo, int verticeInicial) 
{
    ...
    this->listaAdj = new list<int>[getVertices()];
}
...
list<int> * GraphBFS::getListaAdj() const 
{
    return this->listaAdj;
}

void GraphBFS::addAresta(int origem, int destino) 
{
    this->getListaAdj()[origem].push_back(destino);
}

list<int> GraphBFS::getVizinhos(int origem) 
{
...
void GraphBFS::criaListaAdj() 
{
    for(int j = 0; j < getArestas(); j++) 
    {
        ...
        addAresta(verticeOrigem - 1, verticeDestino);
        if(getTipo() == BIDIRECIONAL)
            addAresta(verticeDestino - 1, verticeOrigem);
    }
}
...
vector<bool> GraphBFS::initListFalse() {
    vector<bool> aux;
    for(int i = 0; i < getVertices(); i++) {
        aux.push_back(false);
    }
...
void GraphBFS::BFS(list<int> *ordem) 
{
    vector<bool> visitados = initListFalse();
    queue<int> queue;
    ...
    while(!queue.empty()) 
    {
        ...
        if(!visitados[vertice - 1]) 
        {
            ...
            for(int el: getVizinhos(vertice - 1)) 
            {
                if(!visitados[el - 1]) 
                    queue.push(el);
            }
